...
export default async function create(appName, option) {
  ...
  const tempList = dataRe?.Templates || [];
  ...
  if (option.template) {
    ...
    const template = tempListData.find(
    ...
    if (!template) {
      ...
      console.log(
        `\r\n运行 ${chalk.cyanBright(`${pkg.name} ls`)} 查看所有可用模板\r\n`
      );
      return;
    }
    ...
    const answer = await inquirerChoose("请选择项目模板：", tempListData);
